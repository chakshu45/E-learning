        ...
        const getData = async () => {
            try {
                ...
                const [courseData, lessonsData, enrollmentsData] = await Promise.all([
                    ...
                    fetchData(`/lessons/course/${id}`),
                    fetchData('/enroll/my')
                ]);
                setCourse(courseData);
                setLessons(lessonsData);
                
                const currentEnrollment = enrollmentsData.find((e: any) => e.course._id === id || e.course === id);
    ...
    if (!course) return (
        <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white">
            <div className="text-center">
                ...
                <Link to="/dashboard" className="text-indigo-400 hover:text-indigo-300 underline">Back to Dashboard</Link>
            </div>
        </div>
    );
                        ...
                        <>
                            ...
                            <div className="mt-8 border-t border-slate-800 pt-6 flex items-center justify-between">
                                ...
                                {enrollment?.progress === 100 && (
                                    <Link to={`/courses/${course._id}/certificate`} className="px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl font-bold shadow-lg shadow-orange-500/30 hover:scale-105 transition-all flex items-center">
