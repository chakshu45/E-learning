...
export default function PracticeResults() {
    ...
    useEffect(() => {
        const getResult = async () => {
            try {
                const data = await fetchData(`/tests/result/${id}`);
                setResult(data);
            } catch (error) {
                console.error(error);
            } finally {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950">
                <div className="text-center">
                    ...
                    <Link to="/practice" className="text-indigo-600 font-bold hover:underline">Back to Practice Arena</Link>
                </div>
            </div>
        );
    }
                ...
                <div className="grid md:grid-cols-2 gap-12 mb-16">
                    ...
                    <div className="glass-panel p-8 rounded-[2.5rem] border-slate-100 dark:border-slate-800 flex flex-col justify-center">
                        ...
                        <div className="mt-8 text-center">
                            <Link to="/dashboard" className="inline-block px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-lg shadow-indigo-500/30 hover:scale-105 transition-all w-full text-center">
                                Go to Dashboard
                            </Link>
                        </div>
                    </div>
